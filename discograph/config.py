...
class Configuration(BaseSettings):
    ...
    DEBUG: bool = True
    TESTING: bool = False
    DATA_DIR: Path = ROOT_DIR / "discograph" / "data"
...
class PostgresProductionConfiguration(Configuration):
    ...
    APPLICATION_ROOT: str = "https://discograph.azurewebsites.net/"
    ...
    def model_post_init(self, __context):
        ...
        if self.POSTGRES_DATABASE_USERNAME is None:
            self.POSTGRES_DATABASE_USERNAME = os.getenv("DISCOGRAPH_DATABASE_USERNAME")
        if self.POSTGRES_DATABASE_PASSWORD is None:
            self.POSTGRES_DATABASE_PASSWORD = os.getenv("DISCOGRAPH_DATABASE_PASSWORD")
        if self.POSTGRES_DATABASE_HOST is None:
            self.POSTGRES_DATABASE_HOST = os.getenv("DISCOGRAPH_DATABASE_HOST")
        if self.POSTGRES_DATABASE_PORT is None:
            port_str = os.getenv("DISCOGRAPH_DATABASE_PORT")
            self.POSTGRES_DATABASE_PORT = int(port_str) if port_str else None
        if self.POSTGRES_OFFLINE_DATABASE_NAME is None:
            self.POSTGRES_OFFLINE_DATABASE_NAME = os.getenv("DISCOGRAPH_DATABASE_NAME")
...
class PostgresDevelopmentConfiguration(Configuration):
    ...
    POSTGRES_DATABASE_USERNAME: str = "discograph"
    POSTGRES_DATABASE_PASSWORD: str = "discograph"
    POSTGRES_DATABASE_HOST: str = "localhost"
    POSTGRES_DATABASE_PORT: int = 5432
    POSTGRES_OFFLINE_DATABASE_NAME: str = "discograph_dev"
...
class PostgresTestConfiguration(Configuration):
    ...
    POSTGRES_OFFLINE_DATABASE_NAME: str = "test_offline_discograph"
    POSTGRES_RUNTIME_DATABASE_NAME: str = "test_runtime_discograph"
    ...
    def __init__(self, **data):
        ...
        data["POSTGRES_OFFLINE_DATA"] = (
            TEST_DIR
            / OFFLINE_DATABASE
            / f"discograph_offline_{self._offline_random_string}_test"
        )
        data["POSTGRES_RUNTIME_DATA"] = (
            TEST_DIR
            / RUNTIME_DATABASE
            / f"discograph_runtime_{self._runtime_random_string}_test"
        )
...
class SqliteTestConfiguration(Configuration):
    ...
    def __init__(self, **data):
        ...
        data["SQLITE_OFFLINE_DATABASE_NAME"] = (
            TEST_DIR
            / OFFLINE_DATABASE
            / f"discograph_offline_{self._offline_random_string}_test.db"
        )
        data["SQLITE_RUNTIME_DATABASE_NAME"] = (
            TEST_DIR
            / RUNTIME_DATABASE
            / f"discograph_runtime_{self._runtime_random_string}_test.db"
        )
