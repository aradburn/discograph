...
from sqlalchemy import select, Engine, text, URL, create_engine
...
class CockroachHelper(DatabaseHelper):
    ...
    def get_random_entity(session: Session, roles=None) -> tuple[int, str]:
        ...
        if roles and any(_ not in structural_roles for _ in roles):
            relation = CockroachRelation.get_random_relation(session, roles=roles)
            ...
            while True:
                entity: CockroachEntity = CockroachEntity.get_random_entity(session)
    ...
    def get_relations(session: Session, entity_id: int, entity_type: EntityType):
        ...
        if entity is None:
            return None
        where_clause = CockroachRelation.search(
            entity_id=entity.entity_id,
            entity_type=entity.entity_type,
            query_only=True,
        )
        relations = session.scalars(
            select(CockroachRelation)
            .where(where_clause)
            .order_by(
    ...
    def search_entities(session: Session, search_string):
        ...
        if data is not None:
            ...
            for datum in data["results"]:
                log.debug(f"    {datum}")
            return data
        query = CockroachEntity.build_search_text_query(search_string)
