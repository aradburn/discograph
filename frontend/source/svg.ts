...
type D3Selection = Selection<BaseType, unknown, HTMLElement, undefined>;
type D3SelectionFunction = (selection: D3Selection) => void;
type SVGNodeSelection = Selection<SVGElement, unknown, HTMLElement, undefined>;
...
// Type definitions for global dg object
...
interface DG {
  ...
  svg_dimensions: readonly [number, number];
  network: Network;
}
...
export const initSvg = (): void => {
  ...
  // Initialise tooltip
  const svg = d3.select("#svg") as unknown as SVGNodeSelection;
  (svg.call(nodeToolTip) as unknown as SVGNodeSelection).call(linkToolTip);
};
...
export const setupSvgDefs = (): void => {
  ...
  // RADIAL GRADIENT
  const gradient = defs.append("radialGradient").attr("id", "radial-gradient");
 ...
 */
function saveBlob(dataBlob: Blob, filesize: number): void {
    ...
    Array.from(node.classList).forEach((className) => {
      ...
      if (parentNode instanceof Element) {
        ...
        if (grandParentNode instanceof Element && grandParentNode.id) {
          ...
          Array.from(parentNode.classList).forEach((parentClass) => {
            selectorTextArr.add(`#${grandParentNode.id} .${parentClass}`);
            selectorTextArr.add(
