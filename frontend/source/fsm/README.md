# Discograph FSM Architecture

This directory contains the implementation of the Discograph Finite State Machine (FSM) using the state pattern.
...
- `fsm/`
    ...
    - `DiscographFSM.ts` - Main FSM implementation
...
3. Register the state in the `DiscographFSM` constructor
...
```typescript
// In DiscographFSM.ts constructor
...
1. Update the event handling in `DiscographFSM.handle`
