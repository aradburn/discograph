...
def test_creates_multiselect_mapping_with_valid_data(mocker):
    mocker.patch(
        "discograph.library.cache.role_cache.RoleCache.role_name_to_role_id_lookup",
        {"Producer": 1, "Engineer": 2},
    )
    mocker.patch(
        "discograph.library.cache.role_cache.RoleCache.role_id_to_role_category_lookup",
        {1: "Music", 2: "Sound"},
    )
