...
from discograph.constants import DatabaseType, ThreadingModel, CacheType
...
class TestPydanticConfig(unittest.TestCase):
    ...
    def test_pydantic_sqlite_test_config(self):
        ...
        self.assertTrue(
            "discograph_offline_" in str(config.SQLITE_OFFLINE_DATABASE_NAME)
        )
        self.assertTrue(
            "discograph_runtime_" in str(config.SQLITE_RUNTIME_DATABASE_NAME)
        )
    ...
    def test_pydantic_postgres_test_config(self):
        ...
        self.assertEqual(
            config.POSTGRES_OFFLINE_DATABASE_NAME, "test_offline_discograph"
        )
        self.assertEqual(
            config.POSTGRES_RUNTIME_DATABASE_NAME, "test_runtime_discograph"
        )
    ...
    def test_pydantic_postgres_dev_config(self):
        ...
        # Test PostgreSQL-specific settings
        self.assertEqual(config.POSTGRES_DATABASE_USERNAME, "discograph")
        self.assertEqual(config.POSTGRES_DATABASE_PASSWORD, "discograph")
        self.assertEqual(config.POSTGRES_DATABASE_HOST, "localhost")
        self.assertEqual(config.POSTGRES_DATABASE_PORT, 5432)
        self.assertEqual(config.POSTGRES_OFFLINE_DATABASE_NAME, "discograph_dev")
    ...
    def test_pydantic_env_override(self):
        ...
        orig_username = os.environ.get("DISCOGRAPH_POSTGRES_DATABASE_USERNAME")
        orig_password = os.environ.get("DISCOGRAPH_POSTGRES_DATABASE_PASSWORD")

        try:
            # Set test environment variables
            os.environ["DISCOGRAPH_POSTGRES_DATABASE_USERNAME"] = "test_user"
            os.environ["DISCOGRAPH_POSTGRES_DATABASE_PASSWORD"] = "test_pass"
            ...
            if orig_username:
                os.environ["DISCOGRAPH_POSTGRES_DATABASE_USERNAME"] = orig_username
            else:
                os.environ.pop("DISCOGRAPH_POSTGRES_DATABASE_USERNAME", None)

            if orig_password:
                os.environ["DISCOGRAPH_POSTGRES_DATABASE_PASSWORD"] = orig_password
            else:
                os.environ.pop("DISCOGRAPH_POSTGRES_DATABASE_PASSWORD", None)
