...
class TestSqliteRelationFromRelease(SqliteTestCase):
    def setUp(self):
        super(TestSqliteRelationFromRelease, self).setUp()

    def test_01(self):
        iterator = Bootstrapper.get_iterator("release")
        release_element = next(iterator)
        release_document = SqliteRelease.from_element(release_element)
        release_document.resolve_references({}, spuriously=True)
        actual = SqliteRelation.from_release(release_document)
        expected = [
            {
                "entity_one_id": 41,
